...
ESD_CORE_EXPORT uint32_t Esd_LoadBitmapEx(Esd_BitmapInfo *bitmapInfo, uint8_t *metadata)
{
	...
	if (addr == GA_INVALID)
	{
		...
		if (addr != GA_INVALID)
		{
			...
			int32_t additionalFlashAddr = bitmapInfo->Flash ? Esd_LittleFs_LoadFlashAddress(&bitmapInfo->AdditionalFlashAddress, bitmapInfo->AdditionalFile, NULL) : FA_INVALID;
...
ESD_CORE_EXPORT uint32_t Esd_LoadPalette(Esd_BitmapInfo *bitmapInfo)
{
	...
	if (EVE_CHIPID >= EVE_FT810)
	{
		...
		if (addr == GA_INVALID)
		{
			...
			int32_t paletteFlashAddr = bitmapInfo->Flash ? Esd_LittleFs_LoadFlashAddress(&bitmapInfo->PaletteFlashAddress, bitmapInfo->PaletteFile, NULL) : FA_INVALID;
