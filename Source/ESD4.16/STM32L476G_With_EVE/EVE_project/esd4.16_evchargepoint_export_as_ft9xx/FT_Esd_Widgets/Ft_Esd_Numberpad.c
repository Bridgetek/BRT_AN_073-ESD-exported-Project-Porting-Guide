...
void Ft_Esd_Draw_Display(Ft_Esd_Numberpad *context, int x, int y, int width, int height, bool touching, int ID)
{
	...
	char font = 29;
	int shiftLeft = -6;
	int shiftUp = font / 2 - font;

	// Draw label of the numbers
	EVE_CoCmd_text_ex(phost, x + width + shiftLeft, y + height + shiftUp, font, 0, 0, 0, 0, 0, "1");
	EVE_CoCmd_text_ex(phost, x + (3 * width) + shiftLeft, y + height + shiftUp, font, 0, 0, 0, 0, 0, "2");
	EVE_CoCmd_text_ex(phost, x + (5 * width) + shiftLeft, y + height + shiftUp, font, 0, 0, 0, 0, 0, "3");
	EVE_CoCmd_text_ex(phost, x + width + shiftLeft, y + (3 * height) + shiftUp, font, 0, 0, 0, 0, 0, "4");
	EVE_CoCmd_text_ex(phost, x + (3 * width) + shiftLeft, y + (3 * height) + shiftUp, font, 0, 0, 0, 0, 0, "5");
	EVE_CoCmd_text_ex(phost, x + (5 * width) + shiftLeft, y + (3 * height) + shiftUp, font, 0, 0, 0, 0, 0, "6");
	EVE_CoCmd_text_ex(phost, x + width + shiftLeft, y + (5 * height) + shiftUp, font, 0, 0, 0, 0, 0, "7");
	EVE_CoCmd_text_ex(phost, x + (3 * width) + shiftLeft, y + (5 * height) + shiftUp, font, 0, 0, 0, 0, 0, "8");
	EVE_CoCmd_text_ex(phost, x + (5 * width) + shiftLeft, y + (5 * height) + shiftUp, font, 0, 0, 0, 0, 0, "9");
	EVE_CoCmd_text_ex(phost, x + (3 * width) + shiftLeft, y + (7 * height) + shiftUp, font, 0, 0, 0, 0, 0, "0");
}
...
void Ft_Esd_Connect_Lines(Ft_Esd_Numberpad *context, int curX, int curY)
{
	...
	{
		if (context->isDown)
			Ft_Esd_Render_LineF(((ListOfPoints *)context->Container)->Point[index - 1][0] << 3, ((ListOfPoints *)context->Container)->Point[index - 1][1] << 3,
			    curX << 3, curY << 3, 25, color);
		return;
	}

	for (int x = 0; x < context->PointerIndex - 1; x++)
	{
		Ft_Esd_Render_LineF(((ListOfPoints *)context->Container)->Point[x][0] << 3, ((ListOfPoints *)context->Container)->Point[x][1] << 3,
		    ((ListOfPoints *)context->Container)->Point[x + 1][0] << 3, ((ListOfPoints *)context->Container)->Point[x + 1][1] << 3, 25, color);
	}
